%
% This script makes Figure 9 from:
%
...
%% Get the average responses within ROIs from DKW atlas for all subjects
...
for kk = 1:length(sub_labels)
    ...
    ppgResp = niftiRead(fullfile(dDir,'derivatives','rs_fmri',sub_label,'avgGatedSignalNormalizedPercentage.nii.gz'));

    % load times
    t = load(fullfile(dDir,'derivatives','rs_fmri',sub_label,'avgTimes.mat'));
    
    % load segmentation
    niSegm = niftiRead(fullfile(dDir,'derivatives','rs_fmri',sub_label,'fs_aparc_resampled.nii.gz'));
    % matrix to vector
...
%%
%% Figure 9AB: plot set of responses
...
print('-painters','-r300','-depsc',fullfile(dDir,'derivatives','figures','Figure9AB_allSubs_DKTrois'))
print('-painters','-r300','-dpng',fullfile(dDir,'derivatives','figures','Figure9AB_allSubs_DKTrois'))


%% Figure 9C: plot responses for ACA, PCA, MCA
% colormaps are the same as in figure 4
...
set(gcf,'PaperPositionMode','auto')
print('-painters','-r300','-depsc',fullfile(dDir,'derivatives','figures','Figure9C_ArteryAvgs'))
print('-painters','-r300','-dpng',fullfile(dDir,'derivatives','figures','Figure9C_ArteryAvgs'))

%% Figure 9D: get contrast across subjects 
...
set(gcf,'PaperPositionMode','auto')
print('-painters','-r300','-depsc',fullfile(dDir,'derivatives','figures','Figure9D_ArteryAvgsContrast'))
print('-painters','-r300','-dpng',fullfile(dDir,'derivatives','figures','Figure9D_ArteryAvgsContrast'))
