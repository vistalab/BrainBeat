...
%     niVeno = niftiRead(fullfile(dDir,subj,s_info.veno,[s_info.venoName '.nii.gz']));
    ...
    for scan_nr = 3
        ...
        fmri = fullfile(dDir,['sub-' int2str(s)],'func', [scanName '.nii.gz']);
        ni = niftiRead(fmri);
...
%         ni1.data=ni1.data(:,:,:,1); % use this
        ...
        save(fullfile(dDir,['sub-' int2str(s)],'func',[scanName 'AcpcXform_new.mat']),'acpcXform_new')
    end
...
scan_nr = 3; % scan number from bb_subs
fmri = fullfile(dDir,['sub-' int2str(s)],'func',[s_info.scanName{scan_nr} '.nii.gz']);
ni = niftiRead(fmri);
load(fullfile(dDir,['sub-' int2str(s)],'func',[s_info.scanName{scan_nr} 'AcpcXform_new.mat']),'acpcXform_new')

% Settings for overlay:
curPos = [1,18,35]; 
sliceThisDim = 3; %, z, y, x
