...
% coregister only the te0008, same will be used for te0028

for s = 7 % subject number loop
    s_info = bb_subs(s);
    subj = s_info.subj;
...
%         ni1.data=ni1.data(:,:,:,1); % use this
        ...
        clear scan scanName acpcXform_new
    end
...
scan_nr = 4; % scan number from bb_subs
fmri = fullfile(dDir,subj,s_info.scan{scan_nr},[s_info.scanName{scan_nr} '.nii.gz']);
ni = niftiRead(fmri);
load(fullfile(dDir,subj,s_info.scan{scan_nr},[s_info.scanName{scan_nr} 'AcpcXform_new.mat']),'acpcXform_new')
